...
override public function create():Void
{
	...
	for (i in 0...optionShit.length)
	{
		var menuItem:FlxSprite =
			new FlxSprite(1300, i * 142);

		menuItem.loadGraphic(
			Paths.image('menus/base/titleandmainmenu/menuSelection'),
			true,
			453,
			147
		);
	...
	{
		if (curDifficulty >= existingDifficulties.length)
	...
	{
		diffText = new FlxText(0, 42, 0, '<normal>');
	}
...
override function update(elapsed:Float):Void
{
	...
	/*
		Android safety:
		curAnim can temporarily be null.
	*/
	if (island != null &&
		...
		island.animation.curAnim != null)
	{
		...
		{
			if (island.animation.curAnim.finished &&
				notLoopIsland)
			{
	...
	var up_p:Bool = controls.UP_P;
	var down_p:Bool = controls.DOWN_P;
...
function backToMain():Void
{
	...
	if (FlxG.save.data.storyComplete &&
		!startRandom)
	{
		menuBFGF.visible = true;
		spongeDance.visible = false;
