...
class FreeplayState extends MusicBeatState
{
	...
	public function addSong(songName:String, weekNum:Int, songCharacter:String, songColor:FlxColor)
	{
		...
		{
			trace('ERRO: nenhuma dificuldade encontrada para a música: ' + songName);
		}
	}
	...
	)
	{
		...
		for (song in songs)
		{
			var character:String = songCharacters[Math.min(num[0], songCharacters.length - 1)];
			var color:FlxColor = songColor[Math.min(num[1], songColor.length - 1)];

			addSong(song, weekNum, character, color);
	...
	function transition()
	{
		...
		if (curSelected >= existingDifficulties.length
